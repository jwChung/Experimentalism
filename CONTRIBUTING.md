...
* **코드라인은 120자를 넘지 않아야 합니다.**  
  100자 길이 제한을 권장사항으로 120자 길이를 넘어서는 안됩니다.
  (참고, GitHub의 코드리뷰에서도 **120자** 컬럼 제한규칙을 기본으로 합니다.)
* **현재 코딩되어 있는 스타일을 따라 주세요.**  
  개인마다 선호하는 코딩스타일이 있을 것입니다. 하지만, 현재 Experimentalism에서 코딩되어 있는 스타일을 따라 주시기 바랍니다.
  예를 들어, 필드명은 현재 `this.`를 prefix로 사용하고 있습니다. 따라서 `_필드명`이 아니고 `this.필드명`으로 코딩하여 주시기 바랍니다.
...
* <제목>  
    해당 작업내용의 제목을 작성합니다. 이때, GitHub에서는 제목 라인을 **70자**를 기준으로 이를 초과 시 "..."으로 나타냅니다. 따라서 70자를 초과하지 않게 작성하여야 합니다.
* 공백 줄 추가  
...
### pull request 검증  
Visual studio에서는 release모드에서 모든 테스트들이 성공하고,
솔루션 레벨 Code analysis실행 시(단축키: Alt+F11) 어떠한 경고도 없을 때 이상이 없는
것으로 간주합니다.
