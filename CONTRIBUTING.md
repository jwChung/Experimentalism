...
====================================
Experimentalism 참여방법 설명에 앞서 이 문서는 프로젝트 참여에 대한 제약조건이 됩니다. 따라서 이 문서내용이 빈약할수록 참여하기 쉽다는 뜻과 일맥상통합니다. 하지만 본 오픈소스 프로젝트는 개인을 위한 공간이 아니므로 타인과의 협업을 규칙이 필요합니다. 본 문서에서는 자율과 규칙 사이에서 균형을 찾으려고 노력했습니다.
...
* **현재 코딩되어 있는 스타일을 따라 주세요.**  
  개개인마다 선호하는 코딩스타일이 있을 것입니다. 하지만, 현재 Experimentalism에서 코딩되어 있는 스타일을 따라 주시기 바랍니다. 예를 들어, 필드명은 현재 언더바(_)을 prefix로 사용하고 있습니다. 따라서 `this.필드명`이 아니고 `_필드명`으로 코딩하여 주시기 바랍니다.
...
* <내용>  
 ...
 ### pull request 검증  
    Experimentalism의 루트 디렉토리에는 Run.cmd라는 파일이 있습니다. 이를 실행하여 애러가 발생하지 않으면, 이상이 없는 것으로 판단할 수 있습니다.

    Visual studio에서는 release모드에서 모든 테스트들이 성공하고, 솔루션 레벨 Code analysis실행 시(단축키: Alt+F11) 어떠한 애러나 경고가 없을 시 해당 pull request는 이상이 없는 것을 간주합니다.

Jump in!
--------
Experimentalism 개발 혹은 오픈소스 참여에 익숙하지 않으신 분들은 **Jump in 라벨**이 붙은 이슈를 주목해 주시기 바랍니다. Jump in 라벨의 의미는 다음과 같은 의미를 가집니다.

* 해당 Jump in를 해결하는대 상대적으로 많은 시간이 필요하지 않다.
* 상대적으로 독립적인 이슈로 이해하기 쉽다.
* 해당 이슈를 이해하기 쉽게 잘 설명이 되어 있으며, 필요시 언제든 해당 이슈에 대한 질문을 할 수 있다.

보다 상세한 Jump in 라벨에 대한 자세한 내용은 [여기](http://nikcodes.com/2013/05/10/new-contributor-jump-in/)를 참고하시기 바랍니다.
